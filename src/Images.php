...
class Images
{
	 ...
	 *
	 * @param  array $file Array con el archivo de imagen que queremos copiar
	 * @param  ?string $documentRoot Por defecto $_SERVER["DOCUMENTROOT"]
	 * @param  ?string $path Ruta de destino de la imagen desde el $_SERVER["DOCUMENT_ROOT"]
	 * @param  string $name Nombre que queremos que tenga la imagen, si no se pasa se asigna uno automático
	 * @param  ?int $width Ancho que queremos que tenga la imagen, si no se pasa un alto, esta se redimensaionará en proporción
	 * @param  ?int $height Alto que queremos que tenga la imagen, si no se pasa un ancho esta se redimensionará en proporción
	 * @param  bool $cover Recorta la imagen redimensionada para que cubra completamente al ancho y alto dado
	 * @param  bool $bigResize Fuerza el redimensionado de la imagen a mayor tamaño, por defecto solo redimensaiona a menor tamaño.
	 * @param  stgring $quality Calidad que queremos para la imgen (max|good|medium|low)
	 */
	public function copy(
		...
		$destination = $documentRoot . $path;

		// Create dirs
		$this->create_dirs($path, $documentRoot);
		...
		if ($imgType == "image/jpeg") {
			$copy = $this->copy_jpeg($image, $destination, $bigResize, $this->get_quality($quality, "jpg"));
		} else if ($imgType == "image/png") {
			$copy = $this->copy_png($image, $destination, $bigResize, $this->get_quality($quality, "png"));
		}
	 ...
	 *
	 * @param   string  	$image   		Objeto con la imagen que queremos copiar
	 ...
	 */
	private function copy_jpeg(string $image, string $destination, bool $bigResize = false, int $quality = 80)
	{
		...
		$resize = $this->resize($originalSize, $bigResize);
	 ...
	 * @param   string  	$quality      	(Opcional) Calidad que queremos para la imgen (max|good|medium|low)
	 */
	private function copy_png(string $image, string $destination, bool $bigResize = false, int $quality = 9)
	{
		// Dimensiones de la imagen
	 ...
	 */
	private function create_dirs(string $path, string $documentRoot)
	{
	 ...
	 */
	private function resize($originalSize, $bigResize = false)
	{
		if ($this->height && $this->width) {
			$propsOr = $this->get_proportions($originalSize["ancho"], $originalSize["alto"]);
			$propsRs = $this->get_proportions($this->width, $this->height);

			// Si la tiene las mismas proporciones de lo contrario centramos

			if ($propsOr == $propsRs) {
				return $this->proportionalresize($originalSize, $bigResize);
			} else {
				return $this->centeredresize($originalSize, $bigResize);
			}
		} else {
			return $this->proportionalresize($originalSize, $bigResize);
		}
	 ...
	 */
	private function get_proportions($width, $height)
	{
	 ...
	 * @return 	array						Array con las dimensiones y coordenadas ajustadas.
	 */
	private function proportionalresize($originalSize, $bigResize)
	{
	 ...
	 * @return 	array						Array con las dimensiones y coordenadas ajustadas.
	 */
	private function centeredresize($originalSize, $bigResize)
	{
		...
		if ($resize["ancho"] > $originalSize["ancho"] && $resize["alto"] > $originalSize["alto"] && !$bigResize && !$changeProportions) {
			...
			return $this->centeredresize($originalSize, true);
		}
	 ...
	 */
	private function get_quality($quality = null, $type = null)
	{
		if ($quality == "max") {
